...
public class AHLGame {
	...
	private AHLGame(int id, LocalDate date, Team homeTeam, Team awayTeam) {
		this.id = id;
		this.date = date;
	...
	public static AHLGame parse(BsonDocument jsonScheduleGame) {
		...
		Team awayTeam = Team.parse(Integer.valueOf(
				jsonScheduleGame.getDocument("prop")
						...
						.getString("teamLink")
						.getValue()));
		return new AHLGame(id, date, homeTeam, awayTeam);
	}
