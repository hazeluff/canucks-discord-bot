...
public class GDCVoteAwayCommand extends GDCSubCommand {
	...
	public Publisher<?> reply(ChatInputInteractionEvent event, NHLBot nhlBot, Game game) {
		...
		Prediction prediction = new Prediction(campaignId, userId, game.getGamePk(), game.getAwayTeam().getId());
		SeasonCampaign.savePrediction(nhlBot, prediction);

		String message = String.format("You have voted for the Away team (%s) to win!", game.getAwayTeam().getName());
